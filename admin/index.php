...
// Attempt DB connection — gracefully handle failure
try {
...
<body>
  <div class="container">
      ...
      <nav>
        <ul>
          <?php if (isset($_SESSION['admin_id'])): ?>
            ...
            <li><a href="<?= BASE_PATH ?>/logout.php">Logout (<?= htmlspecialchars($_SESSION['admin_display_name']); ?>)</a></li>
          <?php elseif (isset($_SESSION['judge_id'])): ?>
            <li><a href="<?= BASE_PATH ?>/judge/index.php">Judge Portal</a></li>
            <li><a href="<?= BASE_PATH ?>/logout.php">Logout (<?= htmlspecialchars($_SESSION['judge_display_name']); ?>)</a></li>
          <?php else: ?>
    ...
    <?php if (!$dbConnected): ?>
      <div class="db-error">⚠️ Database Connection Failed. The system is running in offline mode.</div>
    <?php endif; ?>
    ...
    <footer>
      <p>&copy; <?= date('Y'); ?> Event Scoring System</p>
    </footer>
  </div>
</body>
</html>
